<?php
class ControllerExtensionRestapiExtensionPaymentPaymate extends Controller {
	public function index() {
		...
		$data['return'] = $this->url->link('extension/restapi/extension/payment/paymate/callback', 'hash=' . md5($order_info['order_id'] . $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false) . $order_info['currency_code'] . $this->config->get('payment_paymate_password')));
	...
	public function callback() {
		...
		if ($error) {
			...
			$data['text_message'] = sprintf($this->language->get('text_failed_message'), $error, $this->url->link('extension/restapi/information/contact'));
