<?php
class ControllerExtensionRestapiExtensionPaymentKlarnaCheckout extends Controller {
	...
	public function main() {
		...
		$this->session->data['payment_method'] = array(
			...
			'title'      => $text_title,
			'terms'      => $this->url->link('extension/restapi/information/information', 'information_id=' . $this->config->get('klarna_checkout_terms'), true),
			'sort_order' => '1'
		);
	...
	public function validation() {
		...
		if ($validate) {
			...
			$this->response->addHeader('Location: ' . $this->url->link('extension/restapi/checkout/failure', '', true));
		}
		...
		if (isset($this->request->get['klarna_order_id'])) {
			...
			if ($klarna_checkout_order) {
				...
				if ($order_info) {
					...
					if (!$klarna_account || !$connector) {
						$this->model_extension_payment_klarna_checkout->log('Could not getConnector');
						$this->response->redirect($this->url->link('extension/restapi/checkout/failure', '', true));
					}
	...
	private function klarnaOrderData($klarna_account) {
		...
		$merchant_urls = array(
			'checkout'	     => html_entity_decode($this->url->link('extension/restapi/extension/payment/klarna_checkout', 'klarna_order_id={checkout.order.id}', true)),
			'confirmation'   => html_entity_decode($this->url->link('extension/restapi/extension/payment/klarna_checkout/confirmation', 'klarna_order_id={checkout.order.id}', true)),
			'push'			 => html_entity_decode($this->url->link('extension/restapi/extension/payment/klarna_checkout/push', 'klarna_order_id={checkout.order.id}', true)),
			'validation'	 => html_entity_decode($this->url->link('extension/restapi/extension/payment/klarna_checkout/validation', 'klarna_order_id={checkout.order.id}', true)),
			'address_update' => html_entity_decode($this->url->link('extension/restapi/extension/payment/klarna_checkout/addressUpdate', 'klarna_order_id={checkout.order.id}', true)),
			'notification'	 => html_entity_decode($this->url->link('extension/restapi/extension/payment/klarna_checkout/notification', 'klarna_order_id={checkout.order.id}', true)),
		);

		if ($this->config->get('klarna_checkout_terms')) {
			$merchant_urls['terms'] = html_entity_decode($this->url->link('extension/restapi/information/information', 'information_id=' . $this->config->get('klarna_checkout_terms'), true));
		}
