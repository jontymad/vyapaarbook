<?php
class ControllerExtensionRestapiExtensionPaymentGlobalpayRemote extends Controller {
	...
	public function send() {
		...
		if ($this->config->get('payment_globalpay_remote_3d') == 1) {
			if ($this->request->post['cc_type'] == 'visa' || $this->request->post['cc_type'] == 'mc' || $this->request->post['cc_type'] == 'amex') {
				...
				if (isset($verify_3ds->result) && $verify_3ds->result == '00') {
					...
					$json['TermUrl'] = $this->url->link('extension/restapi/extension/payment/globalpay_remote/acsReturn', '', true);
		...
		if ($capture_result->result != '00') {
			...
			$json['success'] = $this->url->link('extension/restapi/checkout/success');
		}
