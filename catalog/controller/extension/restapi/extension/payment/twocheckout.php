<?php
class ControllerExtensionRestapiExtensionPaymentTwoCheckout extends Controller {
	public function index() {
		...
		$data['return_url'] = $this->url->link('extension/restapi/extension/payment/twocheckout/callback', '', true);
	...
	public function callback() {
		...
		if (strtoupper(md5($this->config->get('payment_twocheckout_secret') . $this->config->get('payment_twocheckout_account') . $order_number . $this->request->post['total'])) == $this->request->post['key']) {
			...
			echo '<html>' . "\n";
			echo '<head>' . "\n";
			echo '  <meta http-equiv="Refresh" content="0; url=' . $this->url->link('extension/restapi/checkout/success') . '">' . "\n";
			echo '</head>' . "\n";
			echo '<body>' . "\n";
			echo '  <p>Please follow <a href="' . $this->url->link('extension/restapi/checkout/success') . '">link</a>!</p>' . "\n";
			echo '</body>' . "\n";
			echo '</html>' . "\n";
			exit();
		} else {
