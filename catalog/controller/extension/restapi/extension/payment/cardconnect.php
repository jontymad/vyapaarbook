<?php
class ControllerExtensionRestapiExtensionPaymentCardConnect extends Controller {
	public function index() {
		...
		$data['action'] = $this->url->link('extension/restapi/extension/payment/cardconnect/send', '', true);
					...
					if ($order_info) {
						...
						$this->model_extension_payment_cardconnect->log('Response: ' . print_r($response_data, true));
					 	...
					 	if (isset($response_data['respstat']) && $response_data['respstat'] == 'A') {
							...
							$json['success'] = $this->url->link('extension/restapi/checkout/success', '', true);
						} else {
