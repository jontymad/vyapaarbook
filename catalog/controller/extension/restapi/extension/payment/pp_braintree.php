<?php
class ControllerExtensionRestapiExtensionPaymentPPBraintree extends Controller {
	...
	public function index() {
		...
		$data['payment_url'] = $this->url->link('extension/restapi/extension/payment/pp_braintree/payment', '', true);
		$data['vaulted_url'] = $this->url->link('extension/restapi/extension/payment/pp_braintree/vaulted', '', true);
	...
	public function expressSetup() {
		...
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$json = array();
			$json['error'] = true;
			$json['url'] = $this->url->link('extension/restapi/checkout/cart');
		...
		if (!$this->customer->isLogged() && (!$this->config->get('config_checkout_guest') || $this->config->get('config_customer_price') || $this->cart->hasDownload() || $this->cart->hasRecurringProducts())) {
			$json = array();
			$json['error'] = true;
			$json['url'] = $this->url->link('extension/restapi/checkout/checkout');
	...
	public function expressConfirm() {
		...
		if (isset($this->request->post['coupon']) && $this->validateCoupon()) {
			...
			$this->response->redirect($this->url->link('extension/restapi/extension/payment/pp_braintree/expressConfirm', '', true));
		}
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('extension/restapi/extension/payment/pp_braintree/expressConfirm'),
		...
		$data['action'] = $this->url->link('extension/restapi/extension/payment/pp_braintree/expressConfirm', '', true);
			...
			if (!empty($shipping_address)) {
				...
				if (!empty($results)) {
					...
					if (!empty($quote_data)) {
						...
						$data['action_shipping'] = $this->url->link('extension/restapi/extension/payment/pp_braintree/shipping', '', true);
					} else {
						unset($this->session->data['shipping_methods']);
						unset($this->session->data['shipping_method']);
		...
		$data['action_confirm'] = $this->url->link('extension/restapi/extension/payment/pp_braintree/expressComplete', '', true);
	...
	public function shipping() {
		...
		$this->response->redirect($this->url->link('extension/restapi/extension/payment/pp_braintree/expressConfirm'));
	}
