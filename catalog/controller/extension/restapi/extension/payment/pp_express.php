<?php
class ControllerExtensionRestapiExtensionPaymentPPExpress extends Controller {
	public function index() {
		...
		$data['continue'] = $this->url->link('extension/restapi/extension/payment/pp_express/checkout', '', true);
	...
	public function express() {
		...
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$this->response->redirect($this->url->link('extension/restapi/checkout/cart'));
		}
		...
		if ($this->customer->isLogged()) {
			...
			if ($this->config->get('config_checkout_guest') && !$this->config->get('config_customer_price') && !$this->cart->hasDownload() && !$this->cart->hasRecurringProducts()) {
				...
				$this->response->redirect($this->url->link('extension/restapi/checkout/checkout', '', true));
			}
		}
		...
		$data = array(
			...
			'MAXAMT'             => $max_amount,
			'RETURNURL'          => $this->url->link('extension/restapi/extension/payment/pp_express/expressReturn', '', true),
			'CANCELURL'          => $this->url->link('extension/restapi/checkout/cart', '', true),
	...
	public function expressReturn() {
		...
		$this->response->redirect($this->url->link('extension/restapi/extension/payment/pp_express/expressConfirm', '', true));
	}
	...
	public function expressConfirm() {
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('extension/restapi/extension/payment/pp_express/express'),
			'text' => $this->language->get('text_title')
		);

		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('extension/restapi/extension/payment/pp_express/expressConfirm'),
		...
		$data['action'] = $this->url->link('extension/restapi/extension/payment/pp_express/expressConfirm', '', true);
		...
		foreach ($products as $product) {
			...
			$data['products'][] = array(
				...
				'price'                 => $price,
				'total'                 => $total,
				'href'                  => $this->url->link('extension/restapi/product/product', 'product_id=' . $product['product_id']),
				'remove'                => $this->url->link('extension/restapi/checkout/cart', 'remove=' . $product['cart_id']),
			...
			if (!empty($shipping_address)) {
				...
				if (!empty($results)) {
					...
					if (!empty($quote_data)) {
						...
						$data['action_shipping'] = $this->url->link('extension/restapi/extension/payment/pp_express/shipping', '', true);
					} else {
						unset($this->session->data['shipping_methods']);
						unset($this->session->data['shipping_method']);
		...
		$data['action_confirm'] = $this->url->link('extension/restapi/extension/payment/pp_express/expressComplete', '', true);
	...
	public function expressComplete() {
		...
		if ($this->cart->hasShipping()) {
			...
			if (empty($shipping_address)) {
				$redirect = $this->url->link('extension/restapi/checkout/checkout', '', true);
			}
		...
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$redirect = $this->url->link('extension/restapi/checkout/cart');
		}
		...
		if ($redirect == '') {
			...
			$paypal_data = array(
				...
				'PAYMENTREQUEST_0_NOTIFYURL' => $this->url->link('extension/restapi/extension/payment/pp_express/ipn', '', true),
				'RETURNFMFDETAILS'           => 1
			);
			...
			if ($result['ACK'] == 'Success') {
				...
				$this->response->redirect($this->url->link('extension/restapi/checkout/success'));
	...
	public function checkout() {
		...
		$data = array(
			'METHOD'             => 'SetExpressCheckout',
			'MAXAMT'             => $max_amount,
			'RETURNURL'          => $this->url->link('extension/restapi/extension/payment/pp_express/checkoutReturn', '', true),
			'CANCELURL'          => $this->url->link('extension/restapi/checkout/checkout', '', true),
			'REQCONFIRMSHIPPING' => 0,
			'NOSHIPPING'         => $shipping,
			'LOCALECODE'         => 'EN',
			'LANDINGPAGE'        => 'Login',
	...
	public function checkoutReturn() {
		...
		if ($result['ACK'] == 'Success') {
			...
			$data['continue'] = $this->url->link('extension/restapi/checkout/cart');
	...
	public function shipping() {
		...
		$this->response->redirect($this->url->link('extension/restapi/extension/payment/pp_express/expressConfirm'));
	}
