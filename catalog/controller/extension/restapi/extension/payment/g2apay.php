<?php
class ControllerExtensionRestapiExtensionPaymentG2APay extends Controller {
	public function index() {
		$this->load->language('extension/payment/g2apay');

		$data['action'] = $this->url->link('extension/restapi/extension/payment/g2apay/checkout', '', true);
	...
	public function checkout() {
			...
			if ($this->config->get('total_' . $result['code'] . '_status')) {
				...
				if (isset($order_data['totals'][$i])) {
					if (strstr(strtolower($order_data['totals'][$i]['code']), 'total') === false) {
						...
						$item->url = $this->url->link('extension/restapi/common/home', '', true);
						$items[] = $item;
					}
		...
		foreach ($ordered_products as $product) {
			...
			$item->url = $this->url->link('extension/restapi/product/product', 'product_id=' . $product['product_id'], true);
			$items[] = $item;
		}
		...
		$fields = array(
			...
			'url_failure' => $this->url->link('extension/restapi/checkout/failure'),
			'url_ok' => $this->url->link('extension/restapi/extension/payment/g2apay/success'),
		...
		if ($response_data === false) {
			$this->response->redirect($this->url->link('extension/restapi/extension/payment/failure', '', true));
		}
