<?php
class ControllerExtensionRestapiModuleAmazonPay extends Controller {
	public function index() {
		...
		if ($this->config->get('amazon_login_pay_status') && $this->config->get('amazon_pay_status') && !empty($_SERVER['HTTPS']) && !($this->config->get('amazon_login_pay_minimum_total') > 0 && $this->config->get('amazon_login_pay_minimum_total') > $this->cart->getSubTotal())) {
			...
			$data['amazon_pay_return_url'] = $this->url->link('extension/restapi/module/amazon_pay/login', '', true);
	...
	public function login() {
		...
		if ((array)$user) {
			if (isset($user->error)) {
				...
				$this->response->redirect($this->url->link('extension/restapi/payment/amazon_login_pay/loginFailure', '', true));
			}
			...
			if ($this->customer->isLogged() && $this->customer->getEmail() != $user->email) {
				...
				$this->response->redirect($this->url->link('extension/restapi/payment/amazon_login_pay/address', '', true));
			}
