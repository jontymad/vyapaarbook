<?php
class ControllerExtensionRestapiAccountReturn extends Controller {
	private $error = array();

	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('extension/restapi/account/return', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/restapi/account/return', $url, true)
		);
		...
		foreach ($results as $result) {
			$data['returns'][] = array(
				...
				'href'       => $this->url->link('extension/restapi/account/return/info', 'return_id=' . $result['return_id'] . $url, true)
			);
		}
		...
		$pagination->url = $this->url->link('extension/restapi/account/return', 'page={page}', true);
	...
	public function info() {
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('extension/restapi/account/return/info', 'return_id=' . $return_id, true);
			return new Action('extension/restapi/account/login');
		}
		...
		if ($return_info) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_home'),
				'href' => $this->url->link('extension/restapi/common/home', '', true)
			);
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/restapi/account/return/info', 'return_id=' . $this->request->get['return_id'] . $url, true)
			);
			...
			$data['continue'] = $this->url->link('extension/restapi/account/return', $url, true);
		...
		$this->response->setOutput(json_encode($data));
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/restapi/account/return', '', true)
			);
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_return'),
				'href' => $this->url->link('extension/restapi/account/return/info', 'return_id=' . $return_id . $url, true)
			);
			...
			$data['continue'] = $this->url->link('extension/restapi/account/return', '', true);
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			return new Action('extension/restapi/account/return/success');
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/restapi/account/return/add', '', true)
		);
		...
		$data['action'] = $this->url->link('extension/restapi/account/return/add', '', true);
		...
		if ($this->config->get('config_return_id')) {
			...
			if ($information_info) {
				$data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->link('extension/restapi/information/information/agree', 'information_id=' . $this->config->get('config_return_id'), true), $information_info['title'], $information_info['title']);
			} else {
				$data['text_agree'] = '';
			}
		} else {
